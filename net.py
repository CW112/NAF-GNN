...
class magnn(nn.Module):
    def __init__(self, gcn_depth, num_nodes, device, dropout=0.3, subgraph_size=20, node_dim=40, conv_channels=32, gnn_channels=32, scale_channels=16, end_channels=128, seq_length=12, in_dim=1, out_dim=12, layers=3, propalpha=0.05, tanhalpha=3, single_step=True,model="AP_SAAG"):
        ...
        for i in range(self.layer_num):
            ...
            if model == "MAGNN":
                ...
                self.gconv2.append(mixprop(conv_channels, gnn_channels, gcn_depth, dropout, propalpha))
                # self.gconv1.append(
                #     ARMAK(conv_channels, gnn_channels, gdep=gcn_depth, dropout=dropout))
                # self.gconv2.append(
                #     ARMAK(conv_channels, gnn_channels, gdep=gcn_depth, dropout=dropout))
                # print("运行ARMA")
        ...
        if model == "MAGNN":
            ...
            self.AP_DGNN = AP_DGNN(scale_channels)
            print("运行AP")
        elif model == "SAAG":
            self.SAAG = SAAG()
            print("运行SAAG")
        elif model == "AP_SAAG":
            self.AP_SAAG = AP_SAAG(scale_channels)
            print("运行AP_SAAG")
    ...
    def forward(self, input, idx=None):
        ...
        if self.single_step:
            if self.model == "MAGNN":
                ...
                outputs = self.AP_DGNN(out1)
            elif self.model == "SAAG":
                outputs = self.SAAG(out1)
            elif self.model == "AP_SAAG":
                outputs = self.AP_SAAG(out1)
