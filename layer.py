...
class AP_DGNN(nn.Module):

    def __init__(self, skip_channels):
        super().__init__()

        self.ap_weight = nn.Linear(skip_channels, 1, bias = False)
        self.alpha_param = nn.Parameter(torch.tensor(0.0,dtype=torch.float32, requires_grad=True))

    def reset_parameters(self):
        nn.init.xavier_uniform_(self.ap_weight.weight)

        nn.init.constant_(self.alpha_param, 0.0)
    ...
    def forward(self, x):
        ...
        self_weight_raw = self.ap_weight(x_perm).permute(0, 1, 4, 2, 3)
        self_weight_node = torch.sigmoid(self_weight_raw)
        share_weight = torch.sigmoid(torch.mean(self_weight_raw, dim=3, keepdim=True))
        alpha = self.alpha_param
        ap_weight = self_weight_node * alpha + share_weight * (1 - alpha)
        out = (x * ap_weight).sum(dim=1)
        return out

class SAAG(nn.Module):
    ...
    def forward(self, x):
        ...
        B, K, D, N, C = x.shape
        x_ref = x[:, 0:1, :, :, :]
        saag_weight = torch.sigmoid(
            (x * x_ref).sum(dim=2, keepdim=True) / math.sqrt(D))
        out = (x * saag_weight).sum(dim=1)
        return out



class AP_SAAG(nn.Module):

    def __init__(self, skip_channels, tau=0.5):
        super().__init__()

        self.ap_weight = nn.Linear(skip_channels, 1, bias = False)
        self.learnable_param = nn.Parameter(torch.tensor(torch.zeros(1, 4, 1, 1, 1),dtype=torch.float32, requires_grad=True))
        self.alpha_param = nn.Parameter(torch.tensor(0.0,dtype=torch.float32, requires_grad=True))
        self.k_conv = nn.Conv1d(4, 4, kernel_size=1, bias=True)
    ...
    def forward(self, x):
        ...
        mixed_weight = saag_weight * ap_weight
        mixed_weight = self.k_conv(mixed_weight.squeeze(2).squeeze(-1))
        weight = mixed_weight.unsqueeze(2).unsqueeze(-1) + (self.learnable_param)
...
class ARMAKernel(nn.Module):
    def __init__(self, c_in, c_out, gdep, dropout, activation=F.relu, lambda_k=1.0):
        ...
        self.V = linear(c_in, c_out, bias =  True)
...
class ARMAK(nn.Module):
    def __init__(self, c_in, c_out, gdep = 1, K = 3, dropout = 0.2, activation=F.relu, agg="param"):
        super().__init__()
        self.K = K
        self.agg = agg
        self.kernels = nn.ModuleList([
            ARMAKernel(c_in, c_out, gdep, 0.5, activation)
            for k in range(K)
        ])
        if agg == "param":
            self.alpha = nn.Parameter(torch.tensor(torch.ones(K) / K,dtype=torch.float32, requires_grad=True))
    def forward(self, x, adj):
        outs = []
